import { useForm } from "react-hook-form";
import {loginUser} from '../../api/user'

const LoginForm = () => {

    const usernameConfig = {
        required: true,
        minLength: 3
    }

    const passwordConfig = {
        required: true,
        minLength: 6
    }

    const {
        register,
        handleSubmit,
        formState: {errors}
    } = useForm()

    const onSubmit = async ({username,password}) => {
        const [error, user] = await loginUser(username,password)
        console.log('Error: ', error)
        console.log('User: ',user)
    };
    
    console.log(errors);

    const errorMessage = (() => {
        if(!errors.username){
            return null

        } 
        
        if(errors.username.type === 'required'){
            return <span className="login-err-ms">Username is required</span>

        } 
        
        if(errors.username.type === 'minLength'){
            return <span className="login-err-ms">Username is to short (min 3).</span>

        } 
        
        if(!errors.password){
            return null

        } 
        
        if(errors.password.type === 'required'){
            return <span className="login-err-ms">Password is required</span>

        } 
        
        if(errors.password.type === 'minLength'){
            return <span className="login-err-ms">Password is to short (min 3).</span>

        } 
        
    })()
    return(
        <div id = "login-header">
            <h4 className="header">What is Your Name</h4>
            <form onSubmit={ handleSubmit(onSubmit)}>
                <div>
                    <input type="text" placeholder="Username" {...register("username",usernameConfig)} />
                </div>
                <p>
                    { errorMessage }
                </p>
                <div>
                    <input type="password" placeholder="Password" {...register("password",passwordConfig)} />
                </div>
                <p>
                    { errorMessage }
                </p>
                <div>
                    <button className="btn btn-primary btn-sm" type="submit">Continue</button>
                </div>
            </form>
            
        </div>
    )
};

export default LoginForm;